...
function generateNamedImport(
  ...
  if (
    oneLinerImportStatement.length <= multiLineWrapThreshold &&
  ...
  if (
    ...
    importSpecifierStrings = sortedImportSpecifiers
      .map((o) => `${indent}${symbolSpecifier_1.generateSymbolSpecifier(o)}`)
      .join(',\n');
    ...
    importSpecifierStrings = sortedImportSpecifiers.reduce(
      (acc, curr) => {
        ...
        return {
          out: acc.out + (needsWrap ? `,\n${indent}` : acc.out.length ? `, ` : `${indent}`) + symbolSpecifier,
...
function getImportSpecifiers(namedImport, spaceBraces) {
  ...
  const specifiers = namedImport.specifiers
    .sort(specifierSort)
    .map((o) => symbolSpecifier_1.generateSymbolSpecifier(o))
    .join(', ');
