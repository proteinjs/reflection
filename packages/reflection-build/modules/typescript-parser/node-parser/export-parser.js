...
function parseExport(resource, node) {
  if (TypescriptGuards_1.isExportDeclaration(node)) {
    ...
    if (tsExport.getText().indexOf('*') > -1) {
      ...
      const ex = new NamedExport_1.NamedExport(
        node.getStart(),
        node.getEnd(),
        lib ? lib.text : parse_utilities_1.getDefaultResourceIdentifier(resource)
      );
      ex.specifiers = tsExport.exportClause.elements.map((o) =>
    ...
    if (node.isExportEquals) {
      resource.exports.push(
        new AssignedExport_1.AssignedExport(node.getStart(), node.getEnd(), literal.text, resource)
      );
