...
function parseImport(resource, node) {
  if (TypescriptGuards_1.isImportDeclaration(node)) {
    if (node.importClause && TypescriptGuards_1.isNamespaceImport(node.importClause.namedBindings)) {
      ...
      resource.imports.push(
        new NamespaceImport_1.NamespaceImport(lib.text, alias.text, node.getStart(), node.getEnd())
      );
    } else if (
      node.importClause &&
      (TypescriptGuards_1.isNamedImports(node.importClause.namedBindings) || node.importClause.name)
    ) {
      const lib = node.moduleSpecifier;
      ...
      if (node.importClause.namedBindings) {
        ...
        tsImport.specifiers = bindings.elements.map((o) =>
    ...
    resource.imports.push(
      new ExternalModuleImport_1.ExternalModuleImport(lib.text, alias.text, node.getStart(), node.getEnd())
    );
  }
}
