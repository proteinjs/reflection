...
function parseType(declaration) {
  ...
  if (!typeText) return { text: '', parentTypes: [] };
...
function parseTypeParameters(typeParameterExpression) {
  let serialiedTypeParameters = typeParameterExpression.slice(
    typeParameterExpression.indexOf('<') + 1,
    typeParameterExpression.length - 1
  );
