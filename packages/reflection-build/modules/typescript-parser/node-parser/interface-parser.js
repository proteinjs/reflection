...
function parseInterface(resource, node) {
  ...
  const interfaceDeclaration = new InterfaceDeclaration_1.InterfaceDeclaration(
    name,
    ...
    for (const heritageClause of node.heritageClauses) {
      if (heritageClause.token == typescript_1.SyntaxKind.ExtendsKeyword) {
        for (let heritageType of heritageClause.getChildren()) {
          ...
          interfaceDeclaration.extends.push(parseType_1.parseTypeAndParameters(expressionText));
        }
      }
    }
  }
