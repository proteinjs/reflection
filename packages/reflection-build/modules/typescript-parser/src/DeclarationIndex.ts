...
function getNodeLibraryName(path: string): string {
  ...
  return normalizeFilename(dirs.slice(nodeIndex + 1).join('/')).replace(
...
export class DeclarationIndex {
  ...
  public async reindexForChanges(changes: FileChanges): Promise<IndexDelta> {
    ...
    const resources = await this.parseResources(await this.parser.parseFiles(rebuildFiles, this.rootPath));
  ...
  private async parseResources(files: File[] = []): Promise<Resources> {
    ...
    for (const key of Object.keys(parsedResources).sort((k1, k2) => k2.length - k1.length)) {
      ...
      resource.declarations = resource.declarations.filter((o) => isExportableDeclaration(o) && o.isExported);
  ...
  private async createIndex(resources: Resources): Promise<{ [declaration: string]: DeclarationInfo[] }> {
    ...
    for (const key of Object.keys(resources)) {
      ...
      for (const declaration of resource.declarations) {
        ...
        if (
          !index[declaration.name].some((o) => o.declaration.constructor === declaration.constructor && o.from === from)
        ) {
   ...
   * @memberof DeclarationIndex
   */
  private processNamedFromExport(tsExport: NamedExport, exportingLib: Resource, exportedLib: Resource): void {
   ...
   * @memberof DeclarationIndex
   */
  private processAssignedExport(tsExport: AssignedExport, exportingLib: Resource): void {
    tsExport.exported.forEach((exported) => {
      if (isExportableDeclaration(exported)) {
        ...
        exportingLib.declarations.push(
          ...exported.declarations.filter((o) => isExportableDeclaration(o) && o.isExported)
        );
