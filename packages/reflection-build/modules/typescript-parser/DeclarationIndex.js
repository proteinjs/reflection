...
function getNodeLibraryName(path) {
  ...
  return PathHelpers_1.normalizeFilename(dirs.slice(nodeIndex + 1).join('/')).replace(
    new RegExp(`/(index|${dirs[nodeIndex + 1]}|${dirs[dirs.length - 2]})$`),
    ''
  );
}
/**
...
class DeclarationIndex {
  ...
  static calculateIndexDelta(oldIndex, newIndex) {
    ...
    return {
      ...
      updated: lodash_1.intersection(oldKeys, newKeys).reduce((obj, currentKey) => {
        ...
        if (
          lodash_1.differenceWith(neu, old, lodash_1.isEqual).length > 0 ||
          lodash_1.differenceWith(old, neu, lodash_1.isEqual).length > 0
        ) {
          obj[currentKey] = neu;
        }
        return obj;
  ...
  getExportedResources(resourceToCheck) {
    const resources = [];
    Object.keys(this.parsedResources).forEach((key) => {
  ...
  doesExportResource(resource, resourcePath) {
    ...
    for (const ex of resource.exports) {
      ...
      if (ex instanceof AllExport_1.AllExport || ex instanceof NamedExport_1.NamedExport) {
        const exported =
          '/' +
          PathHelpers_1.toPosix(
            path_1.relative(this.rootPath, path_1.normalize(path_1.join(resource.parsedPath.dir, ex.from)))
          );
    ...
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
      ...
      for (const key of Object.keys(parsedResources).sort((k1, k2) => k2.length - k1.length)) {
        ...
        resource.declarations = resource.declarations.filter(
          (o) => TypescriptHeroGuards_1.isExportableDeclaration(o) && o.isExported
        );
      ...
      for (const key of Object.keys(resources)) {
        ...
        if (resource instanceof Namespace_1.Namespace || resource instanceof Module_1.Module) {
          ...
          index[resource.name].push(
            new DeclarationInfo_1.DeclarationInfo(
              new ModuleDeclaration_1.ModuleDeclaration(resource.getNamespaceAlias(), resource.start, resource.end),
              resource.name
            )
          );
        }
  ...
  processResourceExports(parsedResources, resource, processedResources = []) {
    ...
    for (const ex of resource.exports) {
      if (
        resource instanceof File_1.File &&
        (ex instanceof NamedExport_1.NamedExport || ex instanceof AllExport_1.AllExport)
      ) {
        if (!ex.from) {
          return;
        }
  ...
  processNamedFromExport(tsExport, exportingLib, exportedLib) {
    exportedLib.declarations.forEach((o) => {
    ...
    tsExport.exported.forEach((exported) => {
      if (TypescriptHeroGuards_1.isExportableDeclaration(exported)) {
        ...
        exportingLib.declarations.push(
          ...exported.declarations.filter((o) => TypescriptHeroGuards_1.isExportableDeclaration(o) && o.isExported)
        );
