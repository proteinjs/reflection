...
describe('DeclarationIndex', () => {
  ...
  describe('reindexForChanges()', () => {
    ...
    it('should correctly add a new created file', async () => {
      await declarationIndex.buildIndex([join(rootPath, 'classes.ts')]);
    ...
    it('should correctly update a file that is exported', async () => {
      ...
      await declarationIndex.buildIndex([join(rootPath, 'classes.ts'), join(rootPath, 'foobar.ts')]);
  ...
  describe('exports', () => {
    ...
    it('should parse one single file correctly', async () => {
      await declarationIndex.buildIndex([join(folderRoot, 'classes.ts')]);

      expect(declarationIndex.index).toMatchSnapshot();
    });

    it('should export all elements correctly (export * from)', async () => {
      await declarationIndex.buildIndex([join(folderRoot, 'export-all.ts'), join(folderRoot, 'classes.ts')]);

      expect(declarationIndex.index).toMatchSnapshot();
    });

    it('should export some elements correctly (export {} from)', async () => {
      await declarationIndex.buildIndex([join(folderRoot, 'export-some.ts'), join(folderRoot, 'classes.ts')]);

      expect(declarationIndex.index).toMatchSnapshot();
    });

    it('should export some elements as alias correctly (export {x as y} from)', async () => {
      await declarationIndex.buildIndex([join(folderRoot, 'export-alias.ts'), join(folderRoot, 'classes.ts')]);
