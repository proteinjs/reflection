...
function mapStateToProps(state: AppState, ownProps: Props): Partial<Props> {
  return {
    characters: state.playerCharacters.characters
      ? Object.keys(state.playerCharacters.characters).map((c) => state.playerCharacters.characters![c])
      : [],
    encounters: state.encounters.encounters
      ? Object.keys(state.encounters.encounters).map((c) => state.encounters.encounters![c])
      : null,
...
class EncounterPlanner extends React.Component<Props, State> {
  ...
  public render(): JSX.Element {
    const xp = this.state.monsters.reduce(
      (sum, cur) => (sum += cur.count * challengeRatingExperience[cur.monster.challengeRating || 0]),
      0
    );
      ...
      xp <= 0
        ...
        : startCase(
            Object.keys(thresholds)
              .reverse()
              .find((k) => thresholds[k] <= adjusted) || 'easy'
          );
        ...
        <Toolbar className='toolbar'>
          <ToolbarGroup firstChild={true}>
            <RaisedButton
              ...
              onClick={() => this.setState(EncounterPlanner.defaultState)}
              primary={true}
            />
          </ToolbarGroup>
        ...
        <Row className='content-with-toolbar-top'>
                    ...
                    <Row>
                      <Col xs={6}>
                        ...
                        <Row>
                          ...
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>{thresholds.easy}xp</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>Medium</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>{thresholds.medium}xp</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>Hard</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>{thresholds.hard}xp</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>Deadly</p>
                          </Col>
                          <Col xs={6}>
                            <p style={{ margin: 0 }}>{thresholds.deadly}xp</p>
                          </Col>
                        </Row>
                      </Col>
                      <Col xs={6}>
                        ...
                        <Row>
                          <Col xs={12}>
                            <p style={{ margin: 0 }}>Total EXP</p>
                          </Col>
                          <Col xs={12}>
                            <p style={{ margin: 0, textAlign: 'right' }}>{xp}xp</p>
                          </Col>
                          <Col xs={12}>
                            <p style={{ margin: 0 }}>Adjusted EXP</p>
                          </Col>
                          <Col xs={12}>
                            <p style={{ margin: 0, textAlign: 'right' }}>{adjusted}xp</p>
                          </Col>
                        </Row>
                      </Col>
                    </Row>
                      ...
                      <Row key={key}>
                        <Col xs={12}>
                          ...
                          <p style={{ margin: '0', fontWeight: 'lighter' }}>
                            CR: {monster.monster.challengeRating || 0} XP:{' '}
                            {challengeRatingExperience[monster.monster.challengeRating || 0]}
                          </p>
                        </Col>
                        <Col xs={6}>
                          <TextField
          ...
          <Col xs={8}>
            <div style={containerStyle}>
              <MonstersOverview
                id='encounterPlanner'
                onMonsterSelected={(monster) => this.handleMonsterSelected(monster)}
                showAdd={false}
  ...
  private adjustedXp(xp: number): number {
    ...
    const playerCount =
      this.state.playerCharacters.length + this.state.manualCharacters.reduce((sum, cur) => (sum += cur.count), 0);
