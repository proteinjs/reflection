class CharacterOverview extends React.Component<Props, State> {
  public render(): JSX.Element {
    const chars = sortBy(
      this.props.characters ? Object.keys(this.props.characters).map((key) => this.props.characters![key]) : [],
      ...
      <Row>
        <Col xs={12}>
                  ...
                  {alive && alive.length ? (
                    alive.map((char, idx) => (
                      <TableRow key={idx} style={{ cursor: 'pointer' }}>
                        <TableRowColumn>
                          <span title={char.name}>{char.name}</span>
                        </TableRowColumn>
          ...
          <FloatingActionButton
            ...
            mini={true}
            onClick={() => this.props.navigate!('/characters/new')}
          >
            <ContentAdd />
          </FloatingActionButton>
        </Col>
