...
test('filter foreign interface', async () => {
	const sourceRepository = SourceRepository.get();
  expect(sourceRepository.objects('@proteinjs/reflection-build-test-b/OptionalLoadableForeignInterface')).toHaveLength(0);
});

test('filter foreign abstract class', async () => {
	const sourceRepository = SourceRepository.get();
  expect(sourceRepository.objects('@proteinjs/reflection-build-test-b/OptionalLoadableForeignAbstractClass')).toHaveLength(0);
});