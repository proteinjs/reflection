import { ClassDeclaration as ParserClassDeclaration, DeclarationVisibility } from '../../../modules/typescript-parser';
...
export async function createClassDeclaration(
  parserClassDeclaration: ParserClassDeclaration,
  ...
  for (const accessor of parserClassDeclaration.accessors) {
    if (accessor.isAbstract) isAbstract = true;

    if (accessor.isStatic) isStatic = true;
  ...
  for (const parentInterface of parserClassDeclaration.implements) {
    ...
    for (let parentInterfaceName of parsedInterfaceNames) {
      ...
      directParentInterfaces.push(
        new InterfaceDeclaration(packageName, parentInterfaceName, [], [], typeParameters, [])
      );
    }
  }
  ...
  for (const parentClass of parserClassDeclaration.extends) {
    ...
    directParentClasses.push(
      new ClassDeclaration(packageName, parentClass.name, false, false, 'public', [], [], typeParameters, [], [])
    );
  }

  return new ClassDeclaration(
    packageNameFinder.referencingPackage,
    parserClassDeclaration.name,
    isAbstract,
    isStatic,
    visibility,
    properties,
