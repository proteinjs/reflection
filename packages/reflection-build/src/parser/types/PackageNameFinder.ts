...
export class PackageNameFinder {
  ...
  async getPackageName(symbolName: string): Promise<string> {
    ...
    const relativePathFromSourceFileToPackageJson = path.relative(
      ...
      this.referencingPackageJsonDir
    );
  ...
  private async getLocalPackageName(sourceFilePath: string): Promise<string> {
    ...
    throw new Error(
      `Unable to package-qualify symbol declared in source file: ${sourceFilePath}, reason: unable to find package.json for source file (must be in the same directory as file or a parent directory)`
    );
  }
}
